...
t2max = param([0,0,0],[103,2],0,2)
t2min = param([0,0,0],[103,2],0,3)
par = [fg,lcc,mld,p,t2,t850hPa,tke,u10,u850,u65,v10,v850,v65,ugust,vgust,z500,z1000,z0,r2,t0,t2max,t2min]
...
def generate_grib(start,end,step,points,stations,station_list):
    ...
    while(date != end):
        ...
        p=par[0]
        idx = "{}_{}_{}_{}_{}_{}_{}_{}_{}".format(2,0,p.level[0],p.level[1],p.mbr,p.param[0],p.param[1],p.param[2],p.statistical_process)
        #Check if data for date exists, and if not, continue to next date
        if (idx in index)==False:
            date += step
            continue
        
        #obs = pd.concat([obs_from_smartmet(date,date + datetime.timedelta(hours=66),1,['wmo','time','WSP_PT10M_AVG','WD_PT10M_AVG','WG_PT1H_MAX'],stations['FIN']).rename(columns={"WSP_PT10M_AVG": "WS_PT10M_AVG"}), pd.concat([obs_from_smartmet(date,date + datetime.timedelta(hours=66),1,['wmo','time','WS_PT10M_AVG','WD_PT10M_AVG','WG_PT1H_MAX'],s,'foreign') for k,s in stations.items()])])
        for leadtime in range(67):
            ...
            for country in countries:
                #y = pd.merge(station_list[station_list['Country'] == country],obs.loc[obs['time'] == mydate.strftime('%Y%m%dT%H%M%S')][["wmo",'WS_PT10M_AVG','WD_PT10M_AVG','WG_PT1H_MAX']].rename(columns={"wmo" : "WMON"}),how='left',on='WMON')[['WS_PT10M_AVG','WD_PT10M_AVG','WG_PT1H_MAX']].to_numpy()
                #yield np.transpose(x[country],[0,2,1,3]),y.reshape([1,len(points[country])//4,3]),[leadtime,mydate]
                yield np.transpose(x[country],[0,2,1,3]),[leadtime,mydate]
...
def main():
    ...
    for country in countries:
        ...
        np.savez(f'trainingdata/{country}/{year}/{month}/trainingdata.npz',features=A[country])
